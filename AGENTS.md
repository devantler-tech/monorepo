...
cloud lane's App, which the Cursor loader's measured mapping renders as **`cursor[bot]`** on the REST
and GraphQL APIs and **`app/cursor`** on CLI, PR and search surfaces — *and* its body begins with that identity's canonical `> 🤖 Generated by the`
