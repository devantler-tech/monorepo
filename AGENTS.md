...
**`🤖 Generated with [Claude Code]`** marker. ⚠️ **The two markers are NOT mutually exclusive, and the
interactive one is decisive WHENEVER PRESENT** — an interactive PR can carry the routine disclosure as
well, so a `Generated by the` line is never evidence that a PR cannot be interactive.
...
it as a plain line, none fenced. Six review rounds each found one more container spelling that slipped
past such a detector — an unskipped fence, a nested fence, a blockquoted close token, an indented code
block, a backtick inside an info string, a raw HTML block — so the detector cost an unbounded
enumeration to decide nothing.
...
swallowed by a mis-parsed fence, which drives the maintainer's PR; every round of fence repair risked
reintroducing exactly that. Restore fence handling only against measured incidence of the cheap
...
because it counts bodies using the org PR template, not corpus size.
...
own alike — the conflation that drove `gh pr update-branch` onto two of the maintainer's PRs. On a PR identified as the
