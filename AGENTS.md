...
| **Cursor Bugbot** (`cursor[bot]`) | **STATUS CHECK** with `conclusion: success` — *no review object and no comment at all* | PR comment + inline comments, status check `conclusion: neutral` | check-run `head_sha` == PR head |
...
- **LANE PRIORITY: Codex > Cursor Bugbot > CodeRabbit** (maintainer direction 2026-07-20). Start at
  ...
  | 2 | Cursor Bugbot | `bugbot run` — or `cursor review` (both are exact commands; append `verbose=true` for logs) |
