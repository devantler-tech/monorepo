...
[`coderabbit-summary-verdict.sh --head <headRefOid>`](.claude/scripts/coderabbit-summary-verdict.sh),
...
- **Local review round — when every lane is unavailable OR rate/billing limited** (maintainer
  ...
  **Wait-and-retrigger is still preferred when the wait is short and the run is staying alive** — a
  ...
  and the window lands in the existing auto-generated summary, worded `Next review available in: N
  minutes` on 2026-08-18 and `Next included review available in N minutes` on 2026-09-22. Select
