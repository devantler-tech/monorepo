...
**Line structure, never a bare substring and never a body-start anchor** — measured over 75 open PRs,
line-structural matches exactly the same 5 interactive and 34 routine as a substring match, while a
body-start anchor drops 7 routine to `none`; and unlike a substring match it does **not** fire on a
marker quoted mid-sentence, so a PR *about* this convention cannot park itself HANDS-OFF forever.
