...
2. **Drain oldest-first — and "big" is NOT a reason to skip.** Each run, advance the **oldest
   ...
   **not** want to make issue-level decisions, and a passive "gated / awaiting-maintainer / needs a
   ...
   has an open PR**, don't duplicate it: drive an **actionable trusted-author, non-draft** PR to merge
   per *Merge policy*; leave automation-owned dependency PRs to repository automation, **draft** PRs
   for the maintainer to promote, and **external-contributor** PRs static-review-only and surfaced to
   the maintainer (the trust gate stands — you never merge or run external code).
