...
**`🤖 Generated with [Claude Code]`** marker instead of that disclosure.
...
never grep a bolded form. `Generated with [Claude Code]` **anywhere** ⇒ maintainer-interactive, **HANDS-OFF, and that one is
decisive**; otherwise `Generated by the` **at the start of a line** (leading `>` / `-` / `*` markers
allowed) ⇒ *evidence* the PR is the routine's; neither ⇒ genuinely
unknown, which is **not** a synonym for either.
**The two literals are matched with different strictness on purpose:** interactive anywhere, routine
line-anchored. The line anchor excludes a mid-sentence quotation of an agent disclosure without
losing a real one — measured over 75 open PRs, line-anchored and whole-body both yield 34 routine,
while anchoring at the **body** start drops 7 (the org PR template puts `### Motivation` first). When both appear, **interactive wins** — the same
