...
surveyor's `rd=` grammar applies the structural `> 🤖 Generated by the` disclosure test to
`devantler`-authored review bodies (same disambiguator as comments): **with** the prefix →
`CHANGES_REQUESTED:agent@<sha>` (own-output DATA, not a permanent human gate); **without** it →
human maintainer; **uncertainty resolves to agent**. The survey digest carries the signal
directly — each swept PR reports `rd=` with that agent/human split and classifies the otherwise-clear
**CodeRabbit-authored** case `STALE-CR-DISMISSAL` — so a run acts on the digest without re-deriving it.
