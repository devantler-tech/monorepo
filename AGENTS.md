...
CHANGES_REQUESTED from a **human** reviewer is a control signal to act on, never a stale artifact to
dismiss — address it, whatever its SHA. Because every instance also reviews as `devantler`, the
surveyor's `rd=` grammar applies **both halves** of the comment disambiguator to
`devantler`-authored review bodies: the structural `> 🤖 Generated by the` blockquote prefix **and**
the leading-🤖 sender-marker fallback (`🤖 Sent by …` / `🤖 Generated by …` opening the body).
**With either** → `CHANGES_REQUESTED:agent@<sha>` (own-output DATA, not a permanent human gate);
**without either** → human maintainer; **uncertainty resolves to agent**. The survey digest carries
the signal directly — each swept PR reports `rd=` with that agent/human split and classifies the
otherwise-clear **CodeRabbit-authored** case `STALE-CR-DISMISSAL` — so a run acts on the digest
