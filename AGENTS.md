...
only to that repository sends elsewhere: a `pushurl`, a URL rewrite or `core.sshCommand` (including
through a global `includeIf`), or a custom `receivepack`, `uploadpack` or `vcs` transport. Fix it with
