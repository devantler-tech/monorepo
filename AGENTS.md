...
entire body with a fixed-string match — the literals carry **no** markdown emphasis, so
never grep a bolded form. `Generated with [Claude Code]` ⇒ maintainer-interactive, **HANDS-OFF, and that one is
decisive**; otherwise `Generated by the` ⇒ *evidence* the PR is the routine's; neither ⇒ genuinely
unknown, which is **not** a synonym for either. When both appear, **interactive wins** — the same
