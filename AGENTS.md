...
content, after leading whitespace and any `>` / `-` / `*` markers, begins with the marker —
**after dropping every line inside a ` ``` ` or `~~~` fence**, so a PR that documents this convention
with a fenced example is not read as carrying the marker it merely illustrates. A
...
🔴 **The fence itself is DELIMITER-AWARE, never a boolean toggle.** An opener is a run of **three or
more** ` ``` ` or `~~~`; it closes only on a run of the **same character**, **at least as long** as
the opener, and carrying **no info string**. Track the opening character and its length. A toggle
flips on the first fence token it meets, so an inner `~~~`, a shorter inner run, or an
