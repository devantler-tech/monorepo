...
entire body with a fixed-string match: `Generated with [Claude Code]` ⇒ maintainer-interactive,
HANDS-OFF; otherwise `Generated by the` ⇒ the routine's; neither ⇒ genuinely unknown, which is **not**
a synonym for either. When both appear, **interactive wins** — the same asymmetry stated above, since
reading his PR as yours licenses an unrequested mutation while the reverse merely parks one. Anchoring
