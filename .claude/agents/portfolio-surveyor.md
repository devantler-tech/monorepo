   ...
   **`botantler-1[bot]` is a candidate only for the programmed agent-skills updater classifier**:
   ...
   - **`devantler`-authored PRs: classify the CI state, NOT the ownership — report them as
     ...
     **The ownership test is which literal, never where it sits.** Emit one of three values —
     ...
     - `interactive` — `Generated with [Claude Code]` appears **anywhere** in the body. This is the
       maintainer's own hand-driven session: **HANDS-OFF**.
     - `routine` — no interactive literal, and `Generated by the` appears **at the start of a line**
       (tolerating leading blockquote `>` and list `-`/`*` markers), not merely anywhere in the body.
       Match that STRUCTURAL fragment, never the actor word — it is "Agentic Engineer" now, and
       "Daily AI Engineer" / "Daily AI Assistant" on everything authored before the 2026-07-21 rename.
       🔴 **The asymmetry is deliberate and measured.** A line anchor still excludes mid-sentence
       quotation of an agent disclosure, which is the false-`routine` risk; a **body**-start anchor
       would over-correct, because the org PR template puts `### Motivation` above the disclosure.
       Measured 2026-08-11 over 75 open `devantler` PRs: line-anchored and whole-body both classify
       **34** routine (identical), while a body-start anchor drops **7** of them to `none`.
