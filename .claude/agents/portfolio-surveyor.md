   ...
   **REVIEW-READY**; otherwise **NEEDS-FIX** and name the gate. A `devantler` PR always follows the
   ...
   - **(e) Green-review state per open actionable own/trusted PR — no actionable own/trusted PR is promotion- or
     ...
     **programmed release-bot PRs** (GoReleaser Homebrew-tap cask PRs, KSail release bumps;
     ...
     self-review*): a `devantler`-authored review carrying the `> 🤖 Generated by the Daily AI`
     disclosure and a `## Self-review (fallback` heading, with a `Verdict: no P0/P1 findings` line, at
     a `commit_id` equal to the head. Report a self-review with findings, or at a stale SHA, as
     `none` — only a clean current-head self-review counts.
