   ...
   **`botantler-1[bot]` is a candidate only for the programmed agent-skills updater classifier**:
   ...
   - **`devantler`-authored PRs: classify the CI state, NOT the ownership — report them as
     ...
     **maintainer's interactive** ones (an active feature campaign, `repo-assist`, a hand-driven session):
     ...
     maintainer-interactive PR (`Generated **with** [Claude Code] … in an interactive session`) whose
     ...
     **The ownership test is which literal, never where it sits.** Search the
     **whole** body with a fixed-string match and emit one of three values:
     - `interactive` — the body contains `Generated with [Claude Code]` anywhere. This is the
       maintainer's own hand-driven session: **HANDS-OFF**.
     - `routine` — no interactive literal, and the body contains `Generated by the` anywhere (match
       that STRUCTURAL fragment, never the actor word — it is "Agentic Engineer" now, and
