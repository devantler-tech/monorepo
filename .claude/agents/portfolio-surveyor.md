...
- **`rd` is the PR's `reviewDecision`** (already fetched in the deepening `gh pr view`). When it is
  ...
  test already used for `devantler` comments** to every `devantler`-authored review body (match the
  STRUCTURAL `> 🤖 Generated by the` prefix, any actor word including legacy `Daily AI …` forms):
  - **with** that prefix → **agent output** (own-output DATA), reported as
    `rd=CHANGES_REQUESTED:agent@<sha>` — never as a maintainer control signal;
  - **without** it → **human maintainer**, reported as `rd=CHANGES_REQUESTED:devantler@<sha>`;
  ...
  CHANGES_REQUESTED from a **human** reviewer (`devantler` **without** the disclosure prefix, or any
  other human login) — newest or not — is NEVER stale-dismissable: report it NEEDS-FIX with the
  author named, so the orchestrator addresses the feedback itself. An **agent**-authored
  `CHANGES_REQUESTED:agent@<sha>` is also NEEDS-FIX (own findings to fix-or-refute) but is **not** a
  permanent human gate and is **not** STALE-CR-DISMISSAL.
