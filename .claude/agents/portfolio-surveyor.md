...
4. **CI red on `main` — deployment delta only.** The reviewed plugin surveyor owns current-head
   ...
   `QUERY-UNKNOWN`. **Invoke the classifier only in its flag form:**
   `classify-default-branch-ci-runs.sh --repo OWNER/REPO --branch BRANCH --head-sha FULL_SHA`. The
   helper and the read-only guard accept nothing else: a positional `OWNER/REPO BRANCH SHA` is denied
   as `not the guarded remote-mode shape` and the helper itself exits 2 on it, so the first invocation
   must already carry all three flags (ported verbatim from agent-plugins#195's fix, because this
   overlay is the definition the survey actually reads for this step). The paragraphs below add only
