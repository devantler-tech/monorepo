   ...
   **`botantler-1[bot]` is a candidate only for the programmed agent-skills updater classifier**:
   ...
   - **`devantler`-authored PRs: report the ACTIVE-WORK signals, not an ownership verdict.** The
     ...
     *You own EVERY pull request in the portfolio*, each as an **age**, so the orchestrator applies the
     ...
     `review-envelope`.
     Keep reporting the **branch name** (`headRefName`) and **whether the body leads with a
     `> 🤖 Generated by the …` disclosure** (match the STRUCTURAL prefix, never the actor word — it is
     "Agentic Engineer" now, and "Daily AI Engineer" / "Daily AI Assistant" before the 2026-07-21
     rename; `🤖 Generated **with** [Claude Code]` marks an interactive PR). These stay because the
