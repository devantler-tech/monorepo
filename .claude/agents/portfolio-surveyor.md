...
3. **Deepen only the `devantler`-candidate/trusted-bot pentad candidates.** For the *few* open
   **`devantler`-authored or trusted-bot PRs — drafts and non-drafts —** (`devantler`, `ksail-bot`,
   ...
   — do **not** pull `statusCheckRollup` for every PR in every repo. Classify trusted-bot PRs as
   **MERGE-READY** only when the current-head pentad is clear (CLEAN + required checks/pre-merge green
   + zero threads/body findings + a current-head green review); otherwise **NEEDS-FIX** and name the
   gate. A `devantler` PR always follows the ownership-unverified rule below first.
...
- <repo> #<n> (trusted bot, draft or not) — pentad: checks=<green|failing:X>, unresolved=<n>, body_findings=<n>, premerge=<green|failed:Linked-Issues,…|failed:unnamed|inconclusive|not-posted>, green_review=<cr@<sha>|cr-stale@<sha>|codex@<sha>|codex-stale@<sha>|codex-findings@<sha>|none>, mergeState=<…> → REVIEW-READY | NEEDS-FIX
