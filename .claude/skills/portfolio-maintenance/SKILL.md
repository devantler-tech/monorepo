...
Products → cards: [ksail](../products/ksail/SKILL.md) · [platform](../products/platform/SKILL.md) · [aws](../products/aws/SKILL.md) · [fleet-gitops](../products/fleet-gitops/SKILL.md) ·
...
4. **Clean up:** `.claude/scripts/worktree-remove.sh <path> .claude/worktrees/maint-<runid>` (and prune). Leave
   ...
   overlap a concurrent checkout. Never pass `codex`. This step is what makes the *reap EVERY run*
...
- **Sweep your own disclosure drift — an undisclosed comment you authored reads as a MAINTAINER
  ...
  `> 🤖 Generated by the` prefix as the human maintainer, so every undisclosed review trigger a lane
  posts becomes a standing self-instruction. Run it once per run:

  Sweep **every active repository**, derived live. A literal list goes stale silently and under-reports
  exactly where nobody is looking: the four-repository form this replaced omitted `agent-plugins`, which
  ...
  **Use the sweep wrapper, NOT `comment-disclosure-drift.sh --since` directly.** A bare `--since` call
  ...
  carve-out, because `since` selects by *updated* time, so the returned history is not contiguous and
  adjacency cannot prove the disclosure/trigger pairing. The wrapper applies the contract's consumption
  rule — re-verify **only** a body that is exactly `@cursor review`, with `--issue`, where the full
  comment list is present — and reports what is left. `0` clean, `1` real findings, `2` **UNKNOWN**
  (an unswept repository or an unverifiable carve-out; never read it as clean).

  Each `REAL` row is a hygiene item on the PR it names: it is a violation on sight for
  `@coderabbitai`/`@codex`, which have **no** carve-out — their trigger belongs in the *same* disclosed
  comment as its request marker. Correct it by posting a disclosed superseding comment; never edit the
  maintainer's own comments. Fixing the accumulation matters more than any single row (monorepo#2757).
