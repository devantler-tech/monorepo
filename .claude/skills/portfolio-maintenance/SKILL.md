...
the same cadence (plus after any credential or agent-tooling change), the **local-host
least-privilege audit** (contract *Local agent host*): a read-only pass over token scopes, both
agents' permission/sandbox configs, secret exposure in env/profiles, kube credential scope, and
OS-account privileges; confirmed gaps become `security` children of
[monorepo#2124](https://github.com/devantler-tech/monorepo/issues/2124) (capability classes only,
never secret values). Track it with a `last_host_audit` cursor in memory like the other cadence gates.
