...
EOF
    ...
    if [ "$DH_NOEV" -gt 0 ]; then
      ...
      echo "    RAN, not \"emitted a tool call\": a completed run that called no tool"
      echo "    still had the opportunity to, so it belongs in the denominator — a"
      echo "    tick the agent spent doing nothing is a real observation. Only a"
      echo "    refusal never got that opportunity. Gating this count on tool calls"
      echo "    would over-state every rate, exactly as dropping a truncated run does."
