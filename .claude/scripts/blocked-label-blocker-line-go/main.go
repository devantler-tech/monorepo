...
func askDigestReport(rows []askRow) string {
	...
	// "action", not "decision": the agent owns issue-level decisions, and only
	// a concrete act the maintainer alone can perform belongs here.
	_, _ = fmt.Fprintf(&out, "ASK DIGEST -- %d authority blocker(s) awaiting a maintainer action.\n", len(rows))
