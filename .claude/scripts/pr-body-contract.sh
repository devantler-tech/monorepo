...
validate_body() {
  ...
  routine_disclosure_count="$(awk '
    function normalized(line, indent, rest) {
      sub(/\r$/, "", line)
      while (1) {
        indent = 0
    ...
    {
      line = normalized($0)
      if (line ~ /^Generated by the (Agentic Engineer|Agent Improver|Daily AI Engineer|Daily AI Assistant)[[:space:]]*$/) {
        count++
      }
    }
    END { print count + 0 }
