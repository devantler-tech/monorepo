...
assert_contains "${section}" "**\`cursor[bot]\`** on the REST" \
  'the API spelling of the cloud identity must be accepted, or an API-side lookup rejects its own issue'

# ...and the mapping must match the loader's measured matrix, not an inverted guess. Naming the wrong
# surface sends a checker to the wrong identity, which rejects the authentic tracker or files a duplicate.
assert_contains "${section}" 'and GraphQL APIs' \
  'the identity mapping must follow the measured loader: cursor[bot] on REST and GraphQL, app/cursor on CLI/PR/search'
