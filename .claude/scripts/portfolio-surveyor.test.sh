...
for ln in body.split("\n"):
    if re.match(r"^" + PREFIX + r"(```|~~~)", ln):
        fenced = not fenced
        continue
    if not fenced:
        lines.append(ln)
text = "\n".join(lines)
INTER   = re.compile(r"(?m)^" + PREFIX + r"\U0001F916?[ \t]*Generated with \[Claude Code\]")
ROUTINE = re.compile(r"(?m)^" + PREFIX + r"\U0001F916?[ \t]*Generated by the")
print("interactive" if INTER.search(text) else ("routine" if ROUTINE.search(text) else "none"))
'
}
