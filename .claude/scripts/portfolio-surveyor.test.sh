...
# The harness injects that same literal as a PR-body attribution footer into EVERY Claude session,
# scheduled ones included, so a routine PR that follows it classifies `interactive` and the
# maintainer's steer on the routine's own PR is read as commentary. Measured 2026-09-15: 47 of 79
# `claude/*` PRs since 2026-09-07 carried both markers, 0 of 29 `codex/*`. The fix is at the
# SOURCE (a scheduled run omits the footer; the reminder defers to CLAUDE.md), never a precedence
# flip that would read an interactive PR carrying both markers as the routine's (#3157).
grep -Fq 'A scheduled run never emits this literal' "${constitution}" ||
  fail "AGENTS.md must forbid scheduled runs from emitting the interactive literal (#3157)"
grep -Fq 'the reminder defers to the user' "${constitution}" ||
  fail "AGENTS.md must record that the harness footer reminder yields to this contract (#3157)"
grep -Fq 'never by letting the routine disclosure outrank it' "${constitution}" ||
  fail "AGENTS.md must reject resolving #3157 by flipping marker precedence (#3157)"
...
case "${surveyor_flat}" in
  *'false when a known CI/PR fire or any mandatory-query failure other than the step-4 classifier exists'*'unknown when the step-4 classifier alone failed without a known fire'*'`NOT-DEEPENED (budget)`, `NOT-DEEPENED (next-shard)`, or `DISCOVERY-TRUNCATED (prs, 300 cap)` leaves fire evidence unassessed'*) ;;
