...
for _site in "${surveyor}" "${constitution}"; do
  grep -Fq 'same character' "${_site}" ||
    fail "$(basename "${_site}") must require a closing fence to use the same character (#2762)"
  grep -Fq 'at least as long' "${_site}" ||
    fail "$(basename "${_site}") must require a closing fence run at least as long as the opener (#2762)"
  grep -Fq 'no info string' "${_site}" ||
    fail "$(basename "${_site}") must require a closing fence to carry no info string (#2762)"
done
...
ownership_fixture() {
  ...
  while IFS= read -r ln; do
    ...
    # Process substitution, never a here-document: a PR body legitimately contains backticks, and a
    # here-doc would run them as command substitution against this session's own credentials.
  done < <(printf '%s\n' "${body}")
