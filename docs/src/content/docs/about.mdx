...
export const cvPdf = { href: "/pdfs/nikolai-emil-damm-cv.pdf", filename: "Nikolai Emil Damm - CV.pdf" };

<div style="text-align: center; margin-bottom: 1.5rem;">
  <Image src={profileImg} alt={cv.name} width={280} style="border-radius: 50%;" />
</div>

<div class="cv-download">
  <LinkButton href={cvPdf.href} download={cvPdf.filename} variant="primary" icon="download" iconPlacement="start">
    Download CV (PDF)
  </LinkButton>
</div>
...
<dl class="about-meta">
  <div><dt>Name</dt><dd>{cv.name}</dd></div>
  <div><dt>Location</dt><dd>{cv.location}</dd></div>
  <div><dt>Role</dt><dd>{cv.title}</dd></div>
  <div><dt>Education</dt><dd>{cv.education[0].title}</dd></div>
  <div><dt>Languages</dt><dd>{cv.languages}</dd></div>
  <div><dt>Interests</dt><dd>{cv.interests}</dd></div>
</dl>
...
KSail's public introduction — a mix of conceptual explanation and live demos, showing how it simplifies local development and CI workflows, and how it lets developers take on cluster operations earlier in the process rather than leaving them to the end.

It closes with the roadmap and an open invitation to contribute.

## Professional Experience

### Developer Experience Engineer <span style="float:right">Jun 2025 — Present</span>

_TV2, Odense_

I improve the developer experience for TV2's product teams — working closely with them to find where they get stuck, and with other enabling teams to keep our tools aligned with the wider organization. In May 2026 our team was renamed from **Developer Tooling** to **Development Platform**, as the remit grew past internal tooling to cover running and securing software too.
...
Teaching assistant in Big Data at the University of Southern Denmark. Created course material, taught students, and helped them through their exercises.
...
- Working with Kibana and network monitoring.
...
<dl class="about-meta">
  {cv.technicalSkills.map((skill) => (
    <div><dt>{skill.label}</dt><dd>{skill.value}</dd></div>
  ))}
</dl>
...
<ul class="connect-bar">
  <li><a href={cvPdf.href} download={cvPdf.filename}>Download CV</a></li>
