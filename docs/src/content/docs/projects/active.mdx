...
{/* actions-dirs: aggregate-job-checks,approve-pr,dependency-review,diagnose-flux,enable-auto-merge-on-pr,cleanup-ghcr-packages,free-disk-space,guard-installed-skill-edits,login-to-ghcr,run-dotnet-tests,setup-agent-skills,setup-go-toolchain,setup-ksail-cli,create-issues-from-todos,update-agent-skills,upload-coverage,upsert-issue — one entry per composite-action directory in devantler-tech/actions, in bullet order. The docs CI drift-check asserts this set EQUALS the live action directories, so an added, removed, OR renamed action breaks the build. When that happens, update the matching bullet above and this marker together. */}
